...
y_pred_scales = [[] for m in range(N_MAGNIFICATIONS)]
y_true_scales = [[] for m in range(N_MAGNIFICATIONS)]
...
pred_cancers_scales = [[] for m in range(N_MAGNIFICATIONS)]
pred_hgd_scales = [[] for m in range(N_MAGNIFICATIONS)]
pred_lgd_scales = [[] for m in range(N_MAGNIFICATIONS)]
pred_hyper_scales = [[] for m in range(N_MAGNIFICATIONS)]
pred_normal_scales = [[] for m in range(N_MAGNIFICATIONS)]
...
with torch.no_grad():
	...
	for inputs_bag,labels in testing_generator_bag:
		...
		for b in range(len_bag):
			...
			instances = Dataset_instance(csv_instances,'valid',None)
			...
			predictions, _, _, predictions_scales, _, _ = model(None, features_np, 'multi_scale', None)
			...
			print("acc multi " + str(accuracy_micro(y_true=y_true, y_pred=y_pred)))

			for m in range(N_MAGNIFICATIONS):

				outputs_np = predictions_scales[m].cpu().data.numpy()
				...
				if (TASK=='binary' and N_CLASSES==1):
					...
					pred_cancers_scales[m] = np.append(pred_cancers_scales[m],outputs_np)

				else:

					pred_cancers_scales[m] = np.append(pred_cancers_scales[m],outputs_np[0][0])
					pred_hgd_scales[m] = np.append(pred_hgd_scales[m],outputs_np[0][1])
					pred_lgd_scales[m] = np.append(pred_lgd_scales[m],outputs_np[0][2])
					pred_hyper_scales[m] = np.append(pred_hyper_scales[m],outputs_np[0][3])
					pred_normal_scales[m] = np.append(pred_normal_scales[m],outputs_np[0][4])

				outputs_np = np.where(outputs_np > 0.5, 1, 0)

				y_pred_scales[m] = np.append(y_pred_scales[m],outputs_np)
				y_true_scales[m] = np.append(y_true_scales[m],labels_np)
		j = j+1

#MIXED SCALES

kappa_score_general_filename = checkpoint_path+'kappa_score_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
acc_balanced_filename = checkpoint_path+'acc_balanced_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
acc_filename = checkpoint_path+'acc_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
acc_macro_filename = checkpoint_path+'acc_macro_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
acc_micro_filename = checkpoint_path+'acc_micro_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
confusion_matrix_filename = checkpoint_path+'conf_matr_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
roc_auc_filename = checkpoint_path+'roc_auc_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
f1_score_macro_filename = checkpoint_path+'f1_macro_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
f1_score_micro_filename = checkpoint_path+'f1_micro_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
hamming_loss_filename = checkpoint_path+'hamming_loss_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
recall_score_macro_filename = checkpoint_path+'recall_score_macro_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
recall_score_micro_filename = checkpoint_path+'recall_score_micro_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
jaccard_score_macro_filename = checkpoint_path+'jaccard_score_macro_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
jaccard_score_micro_filename = checkpoint_path+'jaccard_score_micro_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
roc_auc_score_macro_filename = checkpoint_path+'roc_auc_score_macro_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
roc_auc_score_micro_filename = checkpoint_path+'roc_auc_score_micro_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
precision_score_macro_filename = checkpoint_path+'precision_score_macro_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
precision_score_micro_filename = checkpoint_path+'precision_score_micro_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
auc_score_filename = checkpoint_path+'auc_score_MS_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
...
if (TASK=='binary' and N_CLASSES==1):
	...
	filename_training_predictions = checkpoint_path+'WSI_predictions_'+DATASET_TO_SELECT+'_MULTISCALE_'+str(TASK)+'.csv'
...
for m in range(N_MAGNIFICATIONS):
	...
	kappa_score_general_filename = checkpoint_path+'kappa_score_MS_'+current_scale+'_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
	acc_balanced_filename = checkpoint_path+'acc_balanced_MS_'+current_scale+'_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
	acc_filename = checkpoint_path+'acc_MS_'+current_scale+'_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
	acc_macro_filename = checkpoint_path+'acc_macro_MS_'+current_scale+'_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
	acc_micro_filename = checkpoint_path+'acc_micro_MS_'+current_scale+'_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
	confusion_matrix_filename = checkpoint_path+'conf_matr_MS_'+current_scale+'_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
	roc_auc_filename = checkpoint_path+'roc_auc_MS_'+current_scale+'_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
	f1_score_macro_filename = checkpoint_path+'f1_score_MS_'+current_scale+'_macro_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
	f1_score_micro_filename = checkpoint_path+'f1_score_MS_'+current_scale+'_micro_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
	hamming_loss_filename = checkpoint_path+'hamming_loss_MS_'+current_scale+'_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
	recall_score_macro_filename = checkpoint_path+'recall_score_macro_MS_'+current_scale+'_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
	recall_score_micro_filename = checkpoint_path+'recall_score_micro_MS_'+current_scale+'_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
	jaccard_score_macro_filename = checkpoint_path+'jaccard_score_MS_'+current_scale+'_macro_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
	jaccard_score_micro_filename = checkpoint_path+'jaccard_score_MS_'+current_scale+'_micro_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
	roc_auc_score_macro_filename = checkpoint_path+'roc_auc_score_macro_MS_'+current_scale+'_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
	roc_auc_score_micro_filename = checkpoint_path+'roc_auc_score_micro_MS_'+current_scale+'_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
	precision_score_macro_filename = checkpoint_path+'precision_score_macro_MS_'+current_scale+'_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
	precision_score_micro_filename = checkpoint_path+'precision_score_micro_MS_'+current_scale+'_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'
	auc_score_filename = checkpoint_path+'auc_score_'+current_scale+'_'+TASK+'_WSI_'+DATASET_TO_SELECT+'.csv'

	if (TASK=='binary' and N_CLASSES==1):

		acc = metrics.accuracy_score(y_true=y_true_scales[m], y_pred=y_pred_scales[m])
		print("acc " + str(acc))
		save_metric(acc_filename,acc)

		acc_balanced = metrics.balanced_accuracy_score(y_true=y_true_scales[m], y_pred=y_pred_scales[m], sample_weight=None, adjusted=False)
		print("acc_balanced " + str(acc_balanced))
		save_metric(acc_balanced_filename,acc_balanced)

		kappa =  metrics.cohen_kappa_score(y_true_scales[m],y_pred_scales[m])
		print("kappa " + str(kappa))
		save_metric(kappa_score_general_filename,kappa)

		roc_auc_score = metrics.roc_auc_score(y_true_scales[m], y_pred_scales[m])
		print("roc_auc_score " + str(roc_auc_score))
		save_metric(roc_auc_filename,roc_auc_score)

		confusion_matrix = metrics.confusion_matrix(y_true=y_true_scales[m], y_pred=y_pred_scales[m])
		print("confusion_matrix ")
		print(str(confusion_matrix))
		save_metric(confusion_matrix_filename,confusion_matrix)

		try:
			fpr, tpr, thresholds = metrics.roc_curve(y_true_scales[m], y_pred_scales[m], pos_label=1)
			auc_score = metrics.auc(fpr, tpr)
			print("auc_score : " + str(auc_score))
			save_metric(auc_score_filename,auc_score)
		except:
		...
		try:
			target_names = ['cancer', 'hgd', 'lgd', 'hyper']
			classification_report = metrics.classification_report(y_true_scales[m], y_pred_scales[m], target_names=target_names)
			print("classification_report: ")
			print(classification_report)
		except:
			pass
		...
		try:
			f1_score_macro = metrics.f1_score(y_true=y_true_scales[m], y_pred=y_pred_scales[m], average='macro')
			f1_score_micro = metrics.f1_score(y_true=y_true_scales[m], y_pred=y_pred_scales[m], average='micro')
			print("f1_score_macro : " + str(f1_score_macro))
			print("f1_score_micro : " + str(f1_score_micro))
			save_metric(f1_score_macro_filename,f1_score_macro)
			save_metric(f1_score_micro_filename,f1_score_micro)
		...
		try:
			recall_score_macro = metrics.recall_score(y_true=y_true_scales[m], y_pred=y_pred_scales[m], average='macro')
			recall_score_micro = metrics.recall_score(y_true=y_true_scales[m], y_pred=y_pred_scales[m], average='micro')
			print("recall_score_macro : " + str(recall_score_macro))
			print("recall_score_micro : " + str(recall_score_micro))
			save_metric(recall_score_macro_filename,recall_score_macro)
			save_metric(recall_score_micro_filename,recall_score_micro)
		...
		try:
			precision_score_macro = metrics.precision_score(y_true=y_true_scales[m], y_pred=y_pred_scales[m], average='macro')
			precision_score_micro = metrics.precision_score(y_true=y_true_scales[m], y_pred=y_pred_scales[m], average='micro')
			print("precision_score_macro : " + str(precision_score_macro))
			print("precision_score_micro : " + str(precision_score_micro))
			save_metric(precision_score_macro_filename,precision_score_macro)
			save_metric(precision_score_micro_filename,precision_score_micro)
		...
		try:
			roc_auc_score_macro = metrics.roc_auc_score(y_true=y_true_scales[m], y_score=y_pred_scales[m], average='macro')
			roc_auc_score_micro = metrics.roc_auc_score(y_true=y_true_scales[m], y_score=y_pred_scales[m], average='micro')
			print("roc_auc_score_macro : " + str(roc_auc_score_macro))
			print("roc_auc_score_micro : " + str(roc_auc_score_micro)) 
			save_metric(roc_auc_score_macro_filename,roc_auc_score_macro)
			save_metric(roc_auc_score_micro_filename,roc_auc_score_macro)
		...
		filename_training_predictions = checkpoint_path+'WSI_predictions_'+DATASET_TO_SELECT+'_MULTISCALE_ON_SINGLE_SCALE_MAGNIFICATION_'+str(current_scale)+'x_'+str(TASK)+'.csv'

		File = {'filenames':filenames_wsis, 'pred_cancers':pred_cancers_scales[m]}
		...
		y_pred = y_pred_scales[m]
		y_true = y_true_scales[m]
		...
		File = {'filenames':filenames_wsis, 'pred_cancers':pred_cancers_scales[m], 'pred_hgd':pred_hgd_scales[m],'pred_lgd':pred_lgd_scales[m], 'pred_hyper':pred_hyper_scales[m], 'pred_normal':pred_normal_scales[m]}
